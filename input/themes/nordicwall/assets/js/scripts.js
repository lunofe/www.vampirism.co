// Sticky menu logic with IntersectionObserver
document.addEventListener('DOMContentLoaded', function() {
	const header = document.querySelector('.header');
 
	// Check if a sentinel element exists; if not, create it dynamically
	let sentinel = document.querySelector('.sentinel');
	if (!sentinel) {
	  sentinel = document.createElement('div');
	  sentinel.classList.add('sentinel');
	  sentinel.setAttribute('aria-hidden', 'true');
	  // Insert the sentinel directly before the header element
	  header.parentNode.insertBefore(sentinel, header);
	}
 
	// Create an IntersectionObserver with a threshold of 1
	const observer = new IntersectionObserver((entries) => {
	  entries.forEach(entry => {
		 // If the sentinel is not fully visible, the header has reached the top edge (sticky)
...
// Initialize search UI references
...
if (searchButton && searchOverlay) {
    searchButton.addEventListener('click', (e) => {
        ...
        const isExpanded = searchOverlay.classList.toggle('expanded');
        document.body.classList.toggle('is-search-active', isExpanded);
